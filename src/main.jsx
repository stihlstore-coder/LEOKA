...
function WorkforceVisual({ active, onActivate }) {
  ...
  useEffect(() => {
    ...
    const pointBuffers = orbits.map(() => ({ x: 0, y: 0, z: 0 }))
    const geometry = arcs.map((arc) => ({
      primary: makePoints(arc.segments + 1),
      secondary: arc.branchCount === 3 ? [makePoints(8), makePoints(9), makePoints(10)] : [makePoints(8), makePoints(9)],
      micro: arc.branchCount === 3 ? [makePoints(5), makePoints(5), makePoints(5)] : [makePoints(5), makePoints(5)],
      branchEnds: arc.branchCount === 3 ? [{ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }] : [{ x: 0, y: 0 }, { x: 0, y: 0 }],
      microEnds: arc.branchCount === 3 ? [{ x: 0, y: 0 }, { x: 0, y: 0 }, { x: 0, y: 0 }] : [{ x: 0, y: 0 }, { x: 0, y: 0 }],
      pulse: { x: 0, y: 0 },
    }))
    const coronaBuffers = Array.from({ length: 4 }, () => makePoints(9))
    const centerPoint = { x: 0, y: 0 }
    const coreOrbits = [{ rx: 1.9, ry: .34, rot: .48, cycles: 2 }, { rx: 2.25, ry: .24, rot: -.7, cycles: 3 }, { rx: 1.45, ry: .58, rot: 1.1, cycles: 1 }]
    ...
    const centerFor = () => { centerPoint.x = width * .55 + pointerRef.current.x * 4; centerPoint.y = height * .5 + pointerRef.current.y * 4; return centerPoint }
    const pointFor = (orbit, progress, offset = 0, target) => { const theta = orbit.phase + offset + progress * TAU * orbit.cycles; const c = centerFor(); const rawX = Math.cos(theta) * orbit.rx * width; const rawY = Math.sin(theta) * orbit.ry * height; target.x = c.x + rawX * Math.cos(orbit.rot) - rawY * Math.sin(orbit.rot); target.y = c.y + rawX * Math.sin(orbit.rot) + rawY * Math.cos(orbit.rot); target.z = Math.sin(theta + orbit.phase) * orbit.depth; return target }
    ...
    const lightningPath = (from, to, arc, progress, branch = 0, scale = 1, target, segments = arc.segments, seedBase = arc.seed, phaseOffset = arc.phase) => {
      const dx = to.x - from.x; const dy = to.y - from.y; const length = Math.hypot(dx, dy) || 1; const nx = -dy / length; const ny = dx / length
      for (let i = 0; i <= segments; i += 1) {
        const t = i / segments; const envelope = Math.sin(Math.PI * t); const seed = seedBase + branch * 4.13
        const noise = Math.sin(i * 2.37 + seed * 11 + TAU * progress * (2 + branch) + phaseOffset) * 0.58 + Math.sin(i * 5.71 + seed * 7 - TAU * progress * 3) * .27 + Math.sin(i * 9.17 + seed * 13 + TAU * progress * 5) * .15
        const amount = (14 + indexScale(arc) * 13) * scale * envelope * noise
        target[i].x = from.x + dx * t + nx * amount
        target[i].y = from.y + dy * t + ny * amount
      }
      return target
    }
    const indexScale = (arc) => 1 + (arc.segments - 15) * .025
    const drawLightning = (from, to, progress, index, emphasis) => {
      const arc = arcs[index]; const buffer = geometry[index]; const primary = lightningPath(from, to, arc, progress, 0, 1, buffer.primary)
      ...
      for (let branch = 0; branch < arc.branchCount; branch += 1) {
        ...
        branchEnd.x = base.x + normalX * branchDistance + (to.x - from.x) * (.05 + branch * .015)
        branchEnd.y = base.y + normalY * branchDistance + (to.y - from.y) * (.05 + branch * .015)
        const secondary = lightningPath(base, branchEnd, arc, progress + branch * .025, branch + 1, .72, buffer.secondary[branch], 7 + branch, arc.seed + branch * 2.8, arc.phase + branch)
        ...
        if (!mobile) {
          ...
          microEnd.x = microBase.x + (branch % 2 ? 18 : -18)
          microEnd.y = microBase.y + 20 + branch * 5
          const micro = lightningPath(microBase, microEnd, arc, progress + .1, branch + 4, .35, buffer.micro[branch], 4, arc.seed + branch * 8)
      ...
      const pulsePoint = buffer.pulse; pulsePoint.x = px; pulsePoint.y = py
      const glow = context.createRadialGradient(pulsePoint.x, pulsePoint.y, 0, pulsePoint.x, pulsePoint.y, 24); glow.addColorStop(0, 'rgba(255,255,255,.98)'); glow.addColorStop(.12, 'rgba(248,228,255,.95)'); glow.addColorStop(.35, 'rgba(180,116,255,.55)'); glow.addColorStop(1, 'rgba(104,39,220,0)'); context.fillStyle = glow; context.beginPath(); context.arc(pulsePoint.x, pulsePoint.y, 24, 0, TAU); context.fill()
    }
    const draw = (now) => {
      frame = 0
      ...
      context.save(); context.translate(cx, cy); coreOrbits.forEach((orbit) => { context.save(); context.rotate(orbit.rot + phase * orbit.cycles); context.scale(1, orbit.ry); context.beginPath(); context.ellipse(0, 0, coreRadius * orbit.rx, coreRadius * orbit.rx, 0, 0, TAU); context.strokeStyle = 'rgba(214,190,255,.32)'; context.lineWidth = 1; context.setLineDash([2, 7]); context.stroke(); context.restore() }); context.restore()
...
function Workforce() {
  ...
  const nodeStyleCache = useRef([])
  const onNodePosition = useCallback((index, point, reaction, depth) => {
    const node = nodeRefs.current[index]
    if (!node) return
    const scale = .8 + depth * .2 + reaction * .16
    const transform = `translate3d(${point.x - 12}px, ${point.y - 12}px, 0) scale(${scale})`
    const opacity = `${.58 + depth * .25 + reaction * .2}`
    const zIndex = `${10 + Math.round(depth * 10)}`
    const energy = `${1 + reaction * 1.5}`
    const cached = nodeStyleCache.current[index] || (nodeStyleCache.current[index] = {})
    if (cached.transform !== transform) { node.style.transform = transform; cached.transform = transform }
    if (cached.opacity !== opacity) { node.style.opacity = opacity; cached.opacity = opacity }
    if (cached.zIndex !== zIndex) { node.style.zIndex = zIndex; cached.zIndex = zIndex }
    if (cached.energy !== energy) { node.style.setProperty('--energy-hit', energy); cached.energy = energy }
  }, [])
...
function MethodVisual({ active, onActivate }) {
  ...
  useEffect(() => {
    ...
    const planetColors = [
      ['#eee2ff', '#7e46cf', '#170b31'], ['#9883cc', '#39255d', '#090713'],
      ['#f0d7ff', '#7544b8', '#151027'], ['#fff', '#8b4fff', '#2b0b4e'],
      ['#73628f', '#201b38', '#05050b'], ['#e7d6ff', '#5e37a0', '#10091d'],
      ['#fff', '#6a3ab2', '#0d071c'],
    ]
    const makePoints = (count) => Array.from({ length: count }, () => ({ x: 0, y: 0 }))
    const pointBuffers = planets.map(() => ({ x: 0, y: 0, z: 0 }))
    const moonPoints = planets.map(() => ({ x: 0, y: 0 }))
    const lightningPointBuffers = planets.map(() => makePoints(mobile ? 8 : 13))
    const coronaBuffers = Array.from({ length: 6 }, () => makePoints(18))
    const drawOrder = planets.map((planet, index) => ({ planet, point: pointBuffers[index], index }))
    const centerPoint = { x: 0, y: 0 }
    ...
    const center = () => { centerPoint.x = width * (mobile ? .53 : .62); centerPoint.y = height * .55; return centerPoint }
    ...
    const ellipsePoint = (planet, progress, c, target) => { const angle = planet.phase + progress * TAU * planet.speed; const rx = planet.rx * width; const ry = planet.ry * height; const x = Math.cos(angle) * rx; const y = Math.sin(angle) * ry; target.x = c.x + x * Math.cos(planet.tilt) - y * Math.sin(planet.tilt); target.y = c.y + x * Math.sin(planet.tilt) + y * Math.cos(planet.tilt); target.z = Math.sin(angle + planet.phase); return target }
    const drawLightning = (from, to, seed, progress, intensity, points) => { const dx = to.x - from.x; const dy = to.y - from.y; const length = Math.hypot(dx, dy) || 1; const nx = -dy / length; const ny = dx / length; const flash = Math.max(0, Math.sin(TAU * (progress * 3 + seed)) ** 14); const power = intensity * (.12 + flash * .88); const segments = mobile ? 7 : 12; for (let i = 0; i <= segments; i += 1) { const t = i / segments; const wobble = Math.sin(i * 2.7 + seed * 12 + progress * TAU * 2) * Math.sin(Math.PI * t); points[i].x = from.x + dx * t + nx * wobble * 24; points[i].y = from.y + dy * t + ny * wobble * 24 } stroke(points, `rgba(96,31,205,${power * .2})`, 12 * power, 28); stroke(points, `rgba(183,111,255,${power * .5})`, 2.5 * power, 10); stroke(points, `rgba(255,250,255,${power * .78})`, .65 + power, 3) }
    const drawPlanet = (planet, point, index, progress, activeIndex) => { const depth = .74 + point.z * .18; const r = Math.max(10, Math.min(width, height) * .033 * planet.size * depth); const activeEnergy = index === activeIndex ? 1 : .42; const light = ctx.createRadialGradient(point.x - r * .35, point.y - r * .42, r * .08, point.x, point.y, r * 1.35); const color = planetColors[index]; light.addColorStop(0, color[0]); light.addColorStop(.28, color[1]); light.addColorStop(1, color[2]); ctx.save(); ctx.globalAlpha = .62 + depth * .3; ctx.shadowColor = '#9b5cff'; ctx.shadowBlur = 14 + activeEnergy * 14; ctx.fillStyle = light; ctx.beginPath(); ctx.arc(point.x, point.y, r, 0, TAU); ctx.fill(); ctx.shadowBlur = 0; ctx.globalAlpha = .34 + activeEnergy * .42; ctx.strokeStyle = '#d8baff'; ctx.lineWidth = 1; if (index === 0 || index === 4) { ctx.beginPath(); ctx.ellipse(point.x, point.y, r * 1.7, r * .42, planet.tilt, 0, TAU); ctx.stroke() } if (index === 2) { ctx.strokeStyle = '#fff0ff'; for (let line = -1; line < 2; line += 1) { ctx.beginPath(); ctx.moveTo(point.x - r * .7, point.y + line * r * .35); ctx.lineTo(point.x + r * .7, point.y - line * r * .35); ctx.stroke() } } if (index === 3 || index === 6) { const storm = Math.sin(TAU * (progress * 4 + index * .13)); ctx.beginPath(); ctx.arc(point.x - r * .1, point.y, r * (.5 + storm * .12), .3, 2.7); ctx.stroke() } if (index === 1 || index === 5 || index === 6) { const moon = pointForMoon(point, r, index, progress, moonPoints[index]); ctx.fillStyle = '#e9d9ff'; ctx.globalAlpha = .55; ctx.beginPath(); ctx.arc(moon.x, moon.y, Math.max(2, r * .13), 0, TAU); ctx.fill() } ctx.restore(); return r }
    const pointForMoon = (point, radius, index, progress, target) => { const angle = progress * TAU * (2 + index * .17) + index; target.x = point.x + Math.cos(angle) * radius * 2.1; target.y = point.y + Math.sin(angle) * radius * 1.2; return target }
    const draw = (now) => { frame = 0; const rawProgress = reduced ? 0 : (((now - startTime) % LOOP_DURATION) / LOOP_DURATION); const progress = rawProgress >= 1 ? 0 : rawProgress; const phase = progress * TAU; const activeIndex = Math.floor(progress * methodSteps.length) % methodSteps.length; const c = center(); const radius = Math.min(width, height); const pulse = .5 + .5 * Math.sin(phase * 2); if (activeIndex !== lastStage) { lastStage = activeIndex; onActivate(activeIndex) } ctx.clearRect(0, 0, width, height)
      ...
      for (let index = 0; index < planets.length; index += 1) { const planet = planets[index]; const point = ellipsePoint(planet, progress, c, pointBuffers[index]); const depth = clamp(.64 + point.z * .28, .45, 1); const node = nodeRefs.current[index]; if (node) { const transform = `translate3d(${point.x}px, ${point.y}px, 0) translate(-50%, -50%) scale(${depth})`; const opacity = `${.55 + depth * .4}`; const energy = index === activeIndex ? '1' : '.55'; const cached = nodeStyleCache.current[index] || (nodeStyleCache.current[index] = {}); if (cached.transform !== transform) { node.style.transform = transform; cached.transform = transform }; if (cached.opacity !== opacity) { node.style.opacity = opacity; cached.opacity = opacity }; if (cached.energy !== energy) { node.style.setProperty('--planet-energy', energy); cached.energy = energy } } }
      for (let index = 0; index < planets.length; index += 1) { const point = pointBuffers[index]; if (index === activeIndex || index === 3 || index === 4 || index === 6) drawLightning(c, point, hash(index, 22), progress, index === activeIndex ? .8 : .26, lightningPointBuffers[index]) }
      drawOrder.sort((a, b) => a.point.z - b.point.z).forEach(({ planet, point, index }) => drawPlanet(planet, point, index, progress, activeIndex))
      ...
      for (let arc = 0; arc < 6; arc += 1) { const start = phase * (arc % 2 ? -0.35 : .35) + arc * 1.1; const pointsArc = coronaBuffers[arc]; for (let i = 0; i < 18; i += 1) { const t = i / 17; const a = start + (t - .5) * (.8 + .15 * Math.sin(phase + arc)); const rr = sunR * (1.02 + .3 * Math.sin(Math.PI * t) + .06 * Math.sin(phase * 2 + i)); pointsArc[i].x = c.x + Math.cos(a) * rr; pointsArc[i].y = c.y + Math.sin(a) * rr } stroke(pointsArc, 'rgba(168,98,255,.5)', 3.2, 14); stroke(pointsArc, 'rgba(255,247,255,.7)', .75, 3) }
