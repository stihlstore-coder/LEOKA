...
function WorkforceVisual({ active, onActivate }) {
  ...
  useEffect(() => {
    ...
    const coreOrbits = [
      { rx: 1.9, ry: .34, rot: .48, cycles: 2 },
      { rx: 2.25, ry: .24, rot: -.7, cycles: 3 },
      { rx: 1.45, ry: .58, rot: 1.1, cycles: 1 },
    ]
...
function MethodVisual({ active, onActivate }) {
  ...
  useEffect(() => {
    ...
    const ellipsePoint = (planet, progress, c) => { const angle = planet.phase + progress * TAU * planet.speed; const rx = planet.rx * width; const ry = planet.ry * height; const x = Math.cos(angle) * rx; const y = Math.sin(angle) * ry; return { x: c.x + x * Math.cos(planet.tilt) - y * Math.sin(planet.tilt), y: c.y + x * Math.sin(planet.tilt) + y * Math.cos(planet.tilt), z: Math.sin(angle + planet.phase) } }
    ...
    const draw = (now) => { frame = 0; const rawProgress = reduced ? 0 : (((now - startTime) % LOOP_DURATION) / LOOP_DURATION); const progress = rawProgress >= 1 ? 0 : rawProgress; const phase = progress * TAU; const activeIndex = Math.floor(progress * methodSteps.length) % methodSteps.length; const c = center(); const radius = Math.min(width, height); const pulse = .5 + .5 * Math.sin(phase * 2); if (activeIndex !== lastStage) { lastStage = activeIndex; onActivate(activeIndex) } ctx.clearRect(0, 0, width, height)
      ...
      const points = planets.map((planet, index) => { const point = ellipsePoint(planet, progress, c); const depth = clamp(.64 + point.z * .28, .45, 1); const node = nodeRefs.current[index]; if (node) { const transform = `translate3d(${point.x}px, ${point.y}px, 0) translate(-50%, -50%) scale(${depth})`; const opacity = `${.55 + depth * .4}`; const energy = index === activeIndex ? '1' : '.55'; const cached = nodeStyleCache.current[index] || (nodeStyleCache.current[index] = {}); if (cached.transform !== transform) { node.style.transform = transform; cached.transform = transform }; if (cached.opacity !== opacity) { node.style.opacity = opacity; cached.opacity = opacity }; if (cached.energy !== energy) { node.style.setProperty('--planet-energy', energy); cached.energy = energy } } return point })
